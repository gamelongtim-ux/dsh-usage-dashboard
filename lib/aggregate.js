...
function parseSessionFile(file, stat) {
  ...
  let prevTime = null;
  let modelSel = null;
  let sawChunk = false; // 旧格式标记：出现过 assistant/chunk 即为旧版日志
    ...
    for (const line of raw.toString('utf8').split('\n')) {
      ...
      switch (e.type) {
        ...
        case 'assistant/message': {
          ...
          if (!stepInfo[key]) stepInfo[key] = { provider: prov, model, usage: null };
          if (d.usage) stepInfo[key].usage = d.usage; // 双轨：旧版 chunk 与 message 同值，汇总时去重
          const rg = evRange(e);
          // 官方渠道的每次请求都计一次数（无论是否记录到 usage）
          if (prov === 'deepseek-official') {
            const day = noteBoth(e.time);
            const hour = noteHour(e.time);
            day.reqTotal++; hour.reqTotal++;
          }
          // 旧格式（存在 chunk 流）：tokens 延后到旧版分组汇总，避免与 chunk 双计
          if (!sawChunk && d.usage && prov === 'deepseek-official') {
            const u = d.usage;
            const day = noteBoth(e.time);
            const hour = noteHour(e.time);
  ...
  for (const key in stepChunks) {
    ...
    const prov = info.provider || modelSelProvider || null;
    if (prov !== 'deepseek-official') continue; // 非官方渠道不计
    const u = info.usage || g.usage;            // message 与 chunk 双轨同值，取一
    if (!u) continue;
    const input = u.inputTokens || 0, output = u.outputTokens || 0,
      cacheRead = u.cacheReadTokens || 0, reasoning = u.reasoningTokens || 0;
    const tokens = u.totalTokens || (input + output + cacheRead);
    const ts = g.end > 0 ? g.end : (rec.start === Infinity ? g.usage.time : rec.start);
