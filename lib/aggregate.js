...
function blankDay(date) {
  return { date, tokens: 0, input: 0, output: 0, cacheRead: 0, reasoning: 0, userMsgs: 0, toolTotal: 0, durationMs: 0, sessions: 0, byModel: {}, byTool: {} };
}
    ...
    for (const line of raw.toString('utf8').split('\n')) {
      ...
      switch (e.type) {
        ...
        case 'assistant/message': {
          ...
          const rg = evRange(e);
          if (u && (u.outputTokens > 0 || u.inputTokens > 0)) {
            const day = noteBoth(e.time);
            const hour = noteHour(e.time);
            ...
            const bm = day.byModel[model] || (day.byModel[model] = { tokens: 0, input: 0, output: 0, cacheRead: 0, spanMs: 0, spanTokens: 0 });
            const bh = hour.byModel[model] || (hour.byModel[model] = { tokens: 0, input: 0, output: 0, cacheRead: 0, spanMs: 0, spanTokens: 0 });
...
export function aggregate() {
  ...
  for (const rec of recs) {
    ...
    for (const dk in rec.days) {
      ...
      for (const m in src.byModel) {
        ...
        const t = dst.byModel[m] || (dst.byModel[m] = { tokens: 0, input: 0, output: 0, cacheRead: 0, spanMs: 0, spanTokens: 0 });
        for (const k of ['tokens', 'input', 'output', 'cacheRead', 'spanMs', 'spanTokens']) t[k] += s[k];
    ...
    for (let d = new Date(first); d <= last; d.setDate(d.getDate() + 1)) {
      ...
      if (src) {
        days.push({
          ...
          toolCalls: src.toolTotal, sessions: src.sessions, durationMs: src.durationMs, userMsgs: src.userMsgs,
  ...
  const hours = [...hourMap.values()].sort((a, b) => a.date < b.date ? -1 : 1).slice(-48)
    .map(h => ({ date: h.date, tokens: h.tokens, input: h.input, output: h.output, cacheRead: h.cacheRead, toolCalls: h.toolTotal, byModel: h.byModel, byTool: h.byTool }));
