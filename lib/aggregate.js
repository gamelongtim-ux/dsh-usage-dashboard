...
function parseSessionFile(file, stat) {
      ...
      switch (e.type) {
        ...
        case 'assistant/message': {
          ...
          if (u && (u.outputTokens > 0 || u.inputTokens > 0)) {
            ...
            if (rg && output > 0 && rg[1] - rg[0] >= MIN_SPAN_MS) { bm.spanMs += rg[1] - rg[0]; bm.spanTokens += output; }
          }
          break;
        }
  ...
  for (const key in stepChunks) {
    ...
    const bm = day.byModel[model] || (day.byModel[model] = { tokens: 0, input: 0, output: 0, cacheRead: 0, spanMs: 0, spanTokens: 0 });
    bm.tokens += tokens; bm.input += input; bm.output += output; bm.cacheRead += cacheRead;
    if (output > 0 && isFinite(g.start) && g.end - g.start >= MIN_SPAN_MS) { bm.spanMs += g.end - g.start; bm.spanTokens += output; }
  }
